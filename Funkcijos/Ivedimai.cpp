...
void fileIvedimas(vector<Studentas> &studentai, string file)
{
	...
	std::cout << file << " failo nuskaitymas uztruko: " << diff.count() << " s\n";
}
...
void Ivedimas(vector<Studentas> &studentai)
{
	...
	cin >> pavarde;
	Studentas studentas(vardas, pavarde);
	cout << "Ar zinote namu darbu skaiciu? (t-taip, n-ne)" << endl;
	...
	if (ndskz == "t")
	{
		...
		cout << "Ar norite namu darbus ivesti ranka? (t-taip, n-ne): ";
...
void RandomIvedimas(int n)
{
	...
	fd.close();
	std::chrono::duration<double> diff = std::chrono::high_resolution_clock::now() - start; // Skirtumas (s)
	std::cout << "studentai_" << n << " failo sukurimas uztruko: " << diff.count() << " s\n";
}
